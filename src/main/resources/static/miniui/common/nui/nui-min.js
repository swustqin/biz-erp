			...
			success : function(x, q, y) {
				...
				try {
					...
					if (mini_debugger == true) {
						...
						var server_context = location[0]+'//'+location[2]+'/'+location[3];  
						if(location[3].indexOf('.do')>0){
							server_context= location[0]+'//'+location[2];
						}
						//alert(a + "\n json is error.")
						nui.alert("登录已超时，请重新登录！","系统提示",function(text){
							top.location.href=server_context+"/perLogin.do";
							});
					}
				}
				if (z && !mini.isArray(z)) {
...
(function() {
	...
	var a = {
		...
		ajaxLoad : function(d) {
			...
			mini
					.ajax({
						url : server_context+"/dicEntry/getDictData.do",
						data : c,
						type : "POST",
						async : false,
						success : function(f) {
